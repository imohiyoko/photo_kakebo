...
let autoDict: { from: string; to: string }[] = [];
function loadAutoDict() {
  try {
    const p = path.join(process.cwd(), 'db-kakebo', 'ocr_autocorrect.json');
    if (fs.existsSync(p)) autoDict = JSON.parse(fs.readFileSync(p, 'utf8'));
    else autoDict = [];
  } catch {
    autoDict = [];
  }
...
export function applyAutoDict(text: string): string {
  if (!text) return text;
  let out = text;
  autoDict.forEach(e => {
    if (e.from && e.to) {
      try {
        out = out.replace(new RegExp(escapeReg(e.from), 'g'), e.to);
      } catch {}
    }
  });
  return out;
...
function majorityVote(strings: string[]): { value: string | null; count: number; total: number } {
  const f: Record<string, number> = {};
  strings.forEach(s => (f[s] = (f[s] || 0) + 1));
  let best: string | null = null,
    bestCnt = 0;
  Object.entries(f).forEach(([s, c]) => {
    if (c > bestCnt) {
      best = s;
      bestCnt = c;
    }
...
export function aggregateOcr(results: { engine: string; text: string }[]): {
  aggregatedText: string;
  mergedLines: string[];
  conflicts: { lineIndex: number; candidates: string[] }[];
  engines: string[];
} {
  const lineArrays = results.map(r => splitLines(r.text));
  const maxLen = Math.max(...lineArrays.map(a => a.length));
  const merged: string[] = [];
  const conflicts: { lineIndex: number; candidates: string[] }[] = [];
  for (let i = 0; i < maxLen; i++) {
    const candidates = lineArrays.map(a => a[i] || '').filter(c => c.length > 0);
    if (!candidates.length) continue;
    const vote = majorityVote(candidates);
    const isConflict = vote.count < 2 && candidates.length >= 2;
    merged.push(vote.value || '');
    if (isConflict) conflicts.push({ lineIndex: i, candidates });
  }
  return { aggregatedText: merged.join('\n'), mergedLines: merged, conflicts, engines: results.map(r => r.engine) };
}

export function parseOcrText(text: string): { storeName: string | null; purchaseDate: string | null; totalAmount: number | null } {
  ...
  const zen2han = (s: string) => s.replace(/[０-９]/g, ch => String.fromCharCode(ch.charCodeAt(0) - 0xff10 + 0x30));
  const datePattern = /(20[0-9]{2})[\/\-年]\s*([0-9０-９]{1,2})[\/\-月]\s*([0-9０-９]{1,2})日?/;
  const dm = norm.match(datePattern);
  if (dm) {
    const y = dm[1];
    const m = zen2han(dm[2]).padStart(2, '0');
    const d = zen2han(dm[3]).padStart(2, '0');
  ...
  if (filtered.length > 0) {
    result.totalAmount = Math.max(...filtered);
  }
  return result;
}

export function extractItemLines(words: { text: string; bbox: [number, number, number, number] }[]): { text: string; price: string | null; words: typeof words }[] {
  ...
  for (const w of sorted) {
    ...
    if (lastY !== null && Math.abs(cy - lastY) > yThreshold && current.length > 0) {
      lines.push(current);
      current = [];
    }
    current.push(w);
  ...
  return lines.map(ws => {
    const sortedLine = ws.slice().sort((a, b) => a.bbox[0] - b.bbox[0]);
    const text = sortedLine.map(w => w.text).join(' ');
    const priceWord = sortedLine.slice(-1)[0];
    const priceMatch = priceWord.text.match(/[0-9０-９,]+/);
    return { text, price: priceMatch ? priceWord.text.replace(/[,]/g, '') : null, words: sortedLine };
  });
}

function llmResolveConflicts(agg: { mergedLines: string[]; conflicts: { lineIndex: number; candidates: string[] }[] }): { resolutions: any[]; newText: string } {
  const lines = agg.mergedLines.slice();
  const resolutions = [];
  for (const cf of agg.conflicts) {
...
export async function callLLMConflicts(agg: { mergedLines: string[]; conflicts: { lineIndex: number; candidates: string[] }[] }): Promise<{ resolutions: any[]; text: string; latency: number; fallback: boolean }> {
  ...
  const out = j.response || '';
      ...
      let m;
      let ci = 0;
      while ((m = idxRe.exec(out)) !== null && ci < conflictsPayload.length) {
        const chosenIdx = parseInt(m[1], 10);
        const cf = conflictsPayload[ci];
        const chosen = cf.candidates[chosenIdx] || cf.candidates[0];
        lines[cf.lineIndex] = chosen;
        resolutions.push({ lineIndex: cf.lineIndex, resolved: chosen, candidates: cf.candidates });
        ci++;
      }
      for (; ci < conflictsPayload.length; ci++) {
        const cf = conflictsPayload[ci];
        const chosen = cf.candidates.slice().sort((a, b) => b.length - a.length)[0];
        lines[cf.lineIndex] = chosen;
...
export async function runTesseract(imagePath: string): Promise<string> {
  ...
  return applyAutoDict(data.text);
}
