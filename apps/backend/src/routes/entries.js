...
const { diffEntryFields, anonymizeUser } = require('../services/diff');

router.post('/save', (req,res)=>{
  const { id, correctedText, totalAmount, storeName, purchaseDate } = req.body;
  if(!id) return res.status(400).json({ error:'id required'});
  db.run(`UPDATE entries SET corrected_text=?, total_amount=?, store_name=?, purchase_date=? WHERE id=?`, [correctedText, totalAmount, storeName, purchaseDate, id], function(err){
...
router.post('/:id/confirm', (req,res)=>{
  ...
  db.get('SELECT * FROM entries WHERE id=?',[entryId], (err,row)=>{
    ...
    const original = { store_name: row.store_name, purchase_date: row.purchase_date, total_amount: row.total_amount, corrected_text: row.corrected_text };
    const diffs = diffEntryFields(original, edited);
    if(diffs.length){
      const sets=[]; const params=[]; diffs.forEach(d=>{ sets.push(`${d.field_name}=?`); params.push(d.new_value); }); params.push(entryId);
    ...
    diffs.forEach(d=> stmt.run(entryId, d.field_name, d.old_value ?? null, d.new_value ?? null, d.edit_type, null, version, anon));
    stmt.finalize();
    res.json({ status:'ok', diff_count: diffs.length });
  });
});

router.get('/edit-stats', (req,res)=>{
